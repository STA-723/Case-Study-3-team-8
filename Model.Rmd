  ...
  for(e in signifs){
    ...
    d_test <- d_test %>% 
      mutate(bound_low = y_hat - a_cutoff,
             ...
             hit = (bound_low <= Y) & (Y <= bound_upp),
             signif = e)
  
    output <- rbind(output, d_test %>% select(width, hit, signif)) # save output
  } # end-loop signifs
} # end-loop N
...
output %>%
  group_by(signif) %>%
  summarize_all(mean) %>%
  generate_table(name = "conformal.tex", cap = "Width of Prediction Intervals")
